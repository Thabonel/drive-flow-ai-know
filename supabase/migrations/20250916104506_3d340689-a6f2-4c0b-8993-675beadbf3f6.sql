...
-- Remove the problematic RAISE LOG and complete the security enhancements

-- 1. Add security constraint to ensure tokens are encrypted
ALTER TABLE user_google_tokens 
ADD CONSTRAINT IF NOT EXISTS check_encrypted_tokens_not_null 
CHECK (encrypted_access_token IS NOT NULL);

-- 2. Create token cleanup function
...
-- 3. Create encryption key validation function
...
-- 4. Enhanced token access validation with stricter security
...
-- 5. Add policy to block access when encryption is not configured
...
-- 6. Create automatic cleanup trigger
