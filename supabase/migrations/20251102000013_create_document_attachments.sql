-- Create Document Attachments System for Timeline Items
-- Created: 2025-11-02
-- Purpose: Allow users and assistants to attach documents to timeline items
...
CREATE INDEX IF NOT EXISTS idx_timeline_item_documents_timeline_item ON timeline_item_documents(timeline_item_id);
CREATE INDEX IF NOT EXISTS idx_timeline_item_documents_uploaded_by ON timeline_item_documents(uploaded_by_user_id);
CREATE INDEX IF NOT EXISTS idx_timeline_item_documents_assistant ON timeline_item_documents(assistant_relationship_id) WHERE assistant_relationship_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_timeline_item_documents_briefing ON timeline_item_documents(is_briefing_package) WHERE is_briefing_package = TRUE;
CREATE INDEX IF NOT EXISTS idx_timeline_item_documents_created ON timeline_item_documents(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_document_templates_user ON document_templates(user_id);
CREATE INDEX IF NOT EXISTS idx_document_templates_category ON document_templates(category);
CREATE INDEX IF NOT EXISTS idx_document_templates_public ON document_templates(is_public) WHERE is_public = TRUE;
...
CREATE OR REPLACE FUNCTION get_timeline_item_document_count(p_timeline_item_id UUID)
RETURNS INTEGER AS $$
BEGIN
  RETURN (
...
CREATE OR REPLACE FUNCTION get_timeline_item_documents(p_timeline_item_id UUID)
RETURNS TABLE (
  id UUID,
  file_name TEXT,
  file_url TEXT,
...
CREATE OR REPLACE FUNCTION create_briefing_from_template(
...
BEGIN
  ...
  -- Generate new storage path (copy will be handled by application)
  ...
  INSERT INTO timeline_item_documents (
    ...
    v_template.file_url, -- Will be updated after copy
    0, -- Will be updated after copy
...
CREATE OR REPLACE FUNCTION update_timeline_item_documents_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
-- Storage bucket setup instructions (run these in Supabase Dashboard > Storage)
COMMENT ON TABLE timeline_item_documents IS 'Storage bucket "timeline-documents" should be created with the following settings:
- Public: false
- File size limit: 10MB
- Allowed MIME types: application/pdf, application/vnd.openxmlformats-officedocument.wordprocessingml.document, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.openxmlformats-officedocument.presentationml.presentation';
