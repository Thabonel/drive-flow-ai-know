-- Create magnetic_timeline_items table for gap-free 24-hour timeline (FIXED - Idempotent)
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'timeline_templates') THEN
    ...
    IF EXISTS (
      ...
      WHERE constraint_name = 'magnetic_timeline_items_template_id_fkey'
    ) THEN
      ALTER TABLE magnetic_timeline_items
      DROP CONSTRAINT magnetic_timeline_items_template_id_fkey;
    END IF;
...
DROP POLICY IF EXISTS "Users can view own magnetic timeline items" ON magnetic_timeline_items;
CREATE POLICY "Users can view own magnetic timeline items"
  ON magnetic_timeline_items
  FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own magnetic timeline items" ON magnetic_timeline_items;
CREATE POLICY "Users can insert own magnetic timeline items"
  ON magnetic_timeline_items
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own magnetic timeline items" ON magnetic_timeline_items;
CREATE POLICY "Users can update own magnetic timeline items"
  ON magnetic_timeline_items
  FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own magnetic timeline items" ON magnetic_timeline_items;
...
DROP POLICY IF EXISTS "Assistants can view executive magnetic timeline items" ON magnetic_timeline_items;
...
DROP POLICY IF EXISTS "Assistants can insert executive magnetic timeline items" ON magnetic_timeline_items;
...
DROP POLICY IF EXISTS "Assistants can update executive magnetic timeline items" ON magnetic_timeline_items;
...
DROP POLICY IF EXISTS "Assistants can delete executive magnetic timeline items" ON magnetic_timeline_items;
...
DROP TRIGGER IF EXISTS trigger_update_magnetic_timeline_updated_at ON magnetic_timeline_items;
