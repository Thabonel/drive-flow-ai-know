-- Enhanced security measures for Google authentication tokens
...
-- RLS policy for audit log (service roles and users can view their own logs)
...
-- RLS policy for rate limiting (service roles only)
...
BEGIN
  ...
  PERFORM public.log_token_access_attempt(p_user_id, p_action, false, 
...
BEGIN
  ...
  -- Audit log the successful storage
...
-- Function to clean up expired tokens and audit logs
...
DROP POLICY IF EXISTS "Secure tokens view access" ON public.user_google_tokens;
DROP POLICY IF EXISTS "Secure tokens insert access" ON public.user_google_tokens;
DROP POLICY IF EXISTS "Secure tokens update access" ON public.user_google_tokens;
DROP POLICY IF EXISTS "Secure tokens delete access" ON public.user_google_tokens;
...
-- Trigger to automatically clean up data periodically (optional)
...
CREATE TRIGGER cleanup_google_data_trigger
    AFTER INSERT OR UPDATE ON public.user_google_tokens
    FOR EACH ROW
    EXECUTE FUNCTION public.trigger_cleanup_google_data();